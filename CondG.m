...
for i = 1:m
    H(i) = evalh(n,x,i);
    [G(i),flag] = evalg(n,x,i,A,b);
end
F = H + G;
nfev = nfev + 1;

fprintf('----------------------------------------------------------------------\n')
fprintf(' Conditional Gradient Method for Composite Multiojective Optimization \n')
fprintf('----------------------------------------------------------------------\n')
...
while(1)
    ...
    [theta,p,flagIS] = subproblemCondG(n,m,l,u,x,JH,G,dimA,A,b);
    
    %[theta2,p2,flagIS2] = subproblemProxGrad(n,m,l,u,x,JH,G,dimA,A,b);
    %theta2
    ...
    if ( iter > 0 && norm( x - xprev, Inf ) / max(1 , norm( xprev, Inf ) ) <= tol )
        ...
        [thetaPG,pPG,flagISPG] = subproblemProxGrad(n,m,l,u,x,JH,G,dimA,A,b);
